...
class Management(commands.Cog):
    ...
    @commands.command(
        name="enable",
        description="Enables a command/module this is disabled in the guild.",
    )
    ...
    async def enable(self, ctx: commands.Context[Jovanes], *, entity: str) -> Any:
        ...
        if not resolved_entity:  # If no command was found, check whether it's a cog
            ...
            if not resolved_entity:
                e = discord.Embed(
                    description=f"{reactionFailure} No command/module named **{entity}** was found.",
        ...
        async with self.bot.pool.acquire() as conn:
            res = await conn.fetchone(
                "SELECT entity FROM configuration WHERE entity = ? AND guild_id = ?",
                (entity, ctx.guild.id),
            )

            if not res:
                e = discord.Embed(
                    description=f"{reactionFailure} This **{_type}** is not disabled.",
                    color=discord.Color.red(),
                )
                await ctx.reply(embed=e)
                return
            ...
            await conn.execute(
                "DELETE FROM configuration WHERE entity = ? AND guild_id = ?",
                (entity, ctx.guild.id),
            )
    ...
    @commands.command(
        name="disable", description="Renders a command/module unusable in the guild."
    )
    @commands.has_guild_permissions(manage_guild=True)
    @commands.guild_only()
    async def disable(self, ctx: commands.Context[Jovanes], *, entity: str) -> Any:
        ...
        cog = (
            resolved_entity.cog
            if isinstance(resolved_entity, commands.Command)
            else resolved_entity
        )

        if isinstance(cog, Management):  # This cog shouldn't be disabled
            e = discord.Embed(
                description=f"{reactionFailure} Commands in this module or the module itself can't be disabled.",
                color=discord.Color.red(),
            )
            await ctx.reply(embed=e)
            return
        ...
        async with self.bot.pool.acquire() as conn:
            ...
            if res:
                e = discord.Embed(
                    description=f"{reactionFailure} This **{_type}** is already disabled.",
                    color=discord.Color.red(),
                )
                await ctx.reply(embed=e)
                return
            ...
            await conn.execute(
                "INSERT INTO configuration (guild_id, entity, disabled) VALUES (?, ?, ?)",
                (ctx.guild.id, entity, 1),
            )
    ...
    @commands.group(
        name="prefix",
        description="The group for managing prefixes.",
    ...
    async def prefix_add(self, ctx: commands.Context[Jovanes], prefix: str) -> Any:
        ...
        if len(prefix) > 16:
            e = discord.Embed(
                description=f"{reactionFailure} Prefix cannot exceed 16 characters in length.",
                color=discord.Color.red(),
            )
            await ctx.reply(embed=e)
            return
        ...
        if len(prefixes) > 5:
            e = discord.Embed(
                description=f"{reactionFailure} Only five custom prefixes are allowed per guild.",
                color=discord.Color.red(),
            )
            await ctx.reply(embed=e)
            return
        ...
        for _prefix in prefixes:
            if _prefix.strip() == prefix.strip():
                e = discord.Embed(
                    description=f"{reactionFailure} **{prefix}** is already a prefix in this guild.",
                    color=discord.Color.red(),
                )
                await ctx.reply(embed=e)
                return
    ...
    async def prefix_remove(self, ctx: commands.Context[Jovanes]) -> Any:
        ...
        if len(prefixes) == 1:
            e = discord.Embed(
                description=f"{reactionFailure} There is only one prefix left in the server. You can't remove it unless more prefixes are added.",
                color=discord.Color.red(),
            )
            await ctx.reply(embed=e)
            return
    ...
    @commands.command(
        name="setlogging", description="Sets the logging channel for the guild."
    )
    @commands.guild_only()
    @commands.has_guild_permissions(manage_guild=True)
    async def setlogging(
        self, ctx: commands.Context[Jovanes], channel: discord.TextChannel
    ) -> Any:
        assert ctx.guild
        ...
        if not channel.permissions_for(ctx.guild.me).manage_webhooks:
            await ctx.reply(
                embed=discord.Embed(
                    description=f"{reactionFailure} The bot requires permissions to create webhooks in {channel.mention}.",
                    color=discord.Color.red(),
                )
            )
            return
        ...
        async with self.bot.pool.acquire() as conn:
            await conn.execute(
                "UPDATE guild_data SET log_channel = ?, log_webhook = ? WHERE guild_id = ?",
                (channel.id, webhook.url, ctx.guild.id),
            )

...
class TriviaMultiple(discord.ui.View):
    ...
    async def on_timeout(self) -> None:
        ...
        self.embed.add_field(
            name="Status", value=f"{self.user.mention} ran out of time."
        )
        if self.message:
    ...
    async def process_answer(self, interaction: discord.Interaction[Jovanes]) -> None:
        ...
        if clicked_button.label == self.correct_answer:
            ...
            await _utils.update_trivia_score(
                interaction.client, interaction.user.id, True
            )
            self.embed.color = discord.Color.green()
            self.embed.remove_field(3)
            self.embed.add_field(
                name="Status",
                value=f"{interaction.user.mention} got the **correct** answer.",
                inline=False,
            )
        else:
            ...
            for item in self.children:
                if (
                    ...
                    and item.label == self.correct_answer
                ):
                    item.style = discord.ButtonStyle.green

            await _utils.update_trivia_score(
                interaction.client, interaction.user.id, False
            )
            self.embed.color = discord.Color.red()
            self.embed.remove_field(3)
            self.embed.add_field(
                name="Status",
                value=f"{interaction.user.mention} got the **wrong** answer.",
                inline=False,
            )
...
class TriviaBool(discord.ui.View):
    ...
    async def interaction_check(
        self, interaction: discord.Interaction[Jovanes]
    ) -> bool:
        if interaction.user != self.user:
            await interaction.response.send_message(
                "This is not your trivia.", ephemeral=True
            )
            return False
    ...
    async def process_answer(
        self, interaction: discord.Interaction[Jovanes], button: discord.ui.Button
    ) -> None:
        for item in self.children:
            if isinstance(item, discord.ui.Button):
                item.disabled = True
...
class Memory(discord.ui.View):
    def __init__(
        self, bot: Jovanes, user: discord.Member, guild: discord.Guild
    ) -> None:
        self.bot = bot
        self.user = user
        self.guild = guild
        ...
        self.selected_button: Optional[str] = (
            None  # We store the custom_id here, this entire thing works on custom_ids
        )
    ...
    def get_buttons_ready(self) -> None:
        ...
        while i <= 25:
            ...
            if i == 13:  # Ignore the middle part
                self.buttons[unique_id] = discord.ui.Button(
                    label="\u2800",
                ...
                self.buttons[unique_id] = discord.ui.Button(
                    emoji=emoji, style=discord.ButtonStyle.grey, custom_id=unique_id
                )
    ...
    async def start_game(self) -> None:
        ...
        if self.message:
            await self.message.edit(
                content=f"Game has started, {self.user.mention}.", view=self
            )
    ...
    async def check_status(self) -> None:
        if len(self.finished) == 12 and self.message:
            ...
            await self.message.edit(
                content=f"Congrats {self.user.mention}, you finished in {int(minutes)} minute(s) and {int(seconds)} seconds."
            )

            async with self.bot.pool.acquire() as conn:
                res = await conn.fetchone(
                    "SELECT minutes, seconds FROM memory WHERE user_id = ?",
                    (self.user.id),
                )
            ...
            if not res:
                ...
                await conn.execute(
                    "INSERT INTO memory (user_id, minutes, seconds, total_seconds) VALUES (?, ?, ?, ?)",
                    (self.user.id, int(minutes), int(seconds), current_time),
                )
                await conn.commit()
            else:
                ...
                if current_time < saved_time:
                    await conn.execute(
                        "UPDATE memory SET minutes = ?, seconds = ?, total_seconds = ? WHERE user_id = ?",
                        (int(minutes), int(seconds), current_time, self.user.id),
                    )
                    await conn.commit()
    ...
    async def on_timeout(self) -> None:
        ...
        if self.message:
            await self.message.edit(
                content=f"You ran out of time, {self.user.mention}.", view=self
            )
    ...
    async def interaction_check(
        ...
        if interaction.user.id != self.user.id:
            await interaction.response.send_message(
                "This is not your memory game.", ephemeral=True
            )
            return False
    ...
    async def button_callback(self, interaction: discord.Interaction[Jovanes]) -> None:
        ...
        if not self.started:
            await interaction.response.send_message(
                f"Calm down {interaction.user.mention}, game hasn't started yet.",
                ephemeral=True,
            )
            return
...
class TicTacToe(discord.ui.View):
    def __init__(
        self, player_1: discord.Member, player_2: discord.Member, totem: Dict[int, str]
    ) -> None:
    ...
    async def interaction_check(
        ...
        if interaction.user.id != self.current_turn.id:
            await interaction.response.send_message(
                f"This is {self.current_turn.mention}'s turn.", ephemeral=True
            )
            return False
    ...
    async def end_game_timeout(self, bot: Jovanes, message: discord.Message) -> None:
        ...
        async with bot.pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO tictactoe (winner, rival) VALUES (?, ?)",
                (winner.id, rival.id),
            )
    ...
    async def end_game(
        self,
        interaction: discord.Interaction[Jovanes],
    ...
    async def callback(self, interaction: discord.Interaction[Jovanes]) -> None:
        ...
        if clicked_button.label != "\u2800":
            await interaction.response.send_message(
                "This slot has already been marked.", ephemeral=True
            )
            return
        ...
        self.current_turn = (
            self.player_1 if self.player_1.id != interaction.user.id else self.player_2
        )
...
class TicTacToeChallenge(discord.ui.View):
    def __init__(
        self, player: discord.Member, challenged_player: discord.Member
    ) -> None:
        self.player = player
    ...
    async def interaction_check(
        ...
        if interaction.user.id != self.challenged_player.id:
            await interaction.response.send_message(
                f"This can only be used by {self.challenged_player.mention}.",
                ephemeral=True,
            )
            return False
    ...
    async def accept(
        ...
        await interaction.response.edit_message(
            content=f"{self.player.mention} V/S {self.challenged_player.mention}",
            embed=e,
            view=game,
        )
...
class RPS(discord.ui.View):
    def __init__(
        self, player_1: discord.Member, player_2: discord.Member, embed: discord.Embed
    ) -> None:
        self.player_1 = player_1
        self.player_2 = player_2
    ...
    async def interaction_check(
        ...
        if interaction.user.id not in self.players:
            await interaction.response.send_message(
                f"This is a match between {self.player_1.mention} and {self.player_2.mention}, you can't play in this.",
                ephemeral=True,
            )
            return False
    ...
    async def check_win(self, interaction: discord.Interaction[Jovanes]) -> None:
        ...
        self.embed.insert_field_at(
            0,
            name=self.player_1.display_name,
            value=f"{self.get_emoji(self.moves[self.player_1.id])} {self.moves[self.player_1.id].title()}",
        )

        self.embed.remove_field(1)
        self.embed.insert_field_at(
            1,
            name=self.player_2.display_name,
            value=f"{self.get_emoji(self.moves[self.player_2.id])} {self.moves[self.player_2.id].title()}",
        )
        ...
        if winner:
            ...
            async with interaction.client.pool.acquire() as conn:
                await conn.execute(
                    "INSERT INTO rps (winner, rival) VALUES (?, ?)",
                    (winner.id, rival.id),
                )
        else:
            self.embed.description = (
                "Both the players made the same move. Game ended in a draw."
            )
            self.embed.color = discord.Color.greyple()
    ...
    async def rock(
        ...
        if interaction.user.id in self.moves:
            await interaction.response.send_message(
                "You already made your move in this match.", ephemeral=True
            )
            return
        ...
        self.embed.insert_field_at(
            i, name=interaction.user.display_name, value="Moved."
        )
        if len(self.moves) == 2:
    ...
    @discord.ui.button(
        label="Paper", style=discord.ButtonStyle.grey, emoji="\U0001f4c4"
    )
    async def paper(
        self, interaction: discord.Interaction[Jovanes], button: discord.ui.Button
    ) -> None:
        ...
        if len(self.moves) == 2:
            ...
            await interaction.followup.send(
                "You selected :page_facing_up: Paper.", ephemeral=True
            )
    ...
    async def scissors(
        ...
        if len(self.moves) == 2:
            ...
            await interaction.response.edit_message(embed=self.embed, view=self)
            await interaction.followup.send(
                "You selected :scissors: Scissors.", ephemeral=True
            )
...
class GuessModal(discord.ui.Modal):
    def __init__(self, view: Guess) -> None:
        ...
        self.guess = discord.ui.TextInput(
            label="Guess",
            placeholder=f"Type a number in the range {view.min_range} - {view.max_range}",
        )
    ...
    async def on_submit(self, interaction: discord.Interaction[Jovanes]) -> None:
        try:
            ...
            await interaction.response.send_message(
                "Input must be an integer.", ephemeral=True
            )
            return

        if guess < self.view.min_range or guess > self.view.max_range:
            await interaction.response.send_message(
                f"The range is `{self.view.min_range} - {self.view.max_range}`. Your input was `{guess}`.",
                ephemeral=True,
            )
            return
        ...
        if guess == self.view.num:
            ...
            await interaction.response.send_message(
                f"{interaction.user.mention} won the game. The number was {guess}."
            )
            ...
            async with interaction.client.pool.acquire() as conn:
                res = await conn.fetchone(
                    "SELECT win FROM guess WHERE user_id = ?", (interaction.user.id)
                )

                if not res:
                    await conn.execute(
                        "INSERT INTO guess (user_id, wins) VALUES (?, ?)",
                    ...
                    await conn.execute(
                        "UPDATE guess SET wins = ? WHERE user_id = ?",
                        (wins, interaction.user.id),
                    )
            ...
            await interaction.response.send_message(
                f"Your guess was wrong.", ephemeral=True
            )
            log = self.view.embed.fields[2].value
            assert log is not None
...
class Guess(discord.ui.View):
    ...
    async def end_game(self) -> None:
        ...
        if self.message:
            await self.message.edit(
                content=f"No one guessed the correct number. It was `{self.num}`.",
                view=self,
            )
    ...
    async def guess(
        ...
        if self.guesses[interaction.user.id] == self.no_of_guesses:
            await interaction.response.send_message(
                "You reached your maximum guesses.", ephemeral=True
            )
            return
...
class TextPaginator(discord.ui.View):
    ...
    async def rewind(
        ...
        await interaction.response.edit_message(
            content=self.split_parts[self.current_page - 1], view=self
        )
...
class PrefixRemoveSelect(discord.ui.Select["PrefixRemove"]):
    def __init__(self, prefixes: List[str]) -> None:
        ...
        options = [
            discord.SelectOption(label=prefix, value=prefix) for prefix in prefixes
        ]
    ...
    async def callback(self, interaction: discord.Interaction[Jovanes]) -> Any:
        ...
        async with interaction.client.pool.acquire() as conn:
            ...
            await conn.execute(
                ...
                (interaction.guild.id, actual_prefix),
            )

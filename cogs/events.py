...
class Events(commands.Cog):
    ...
    async def on_command_error(
        self, ctx: commands.Context[Jovanes], exception: errors.CommandError
    ) -> Any:
        ...
        if isinstance(exception, EntityDisabled):
            ...
            e = discord.Embed(
                description=f"{reactionFailure} This {_type} is currently disabled.",
        ...
        if isinstance(exception, errors.ExpectedClosingQuoteError):
            e = discord.Embed(
                description=f"{reactionFailure} No closing {exception.close_quote} was found.",
                color=discord.Color.red(),
            )
            return await ctx.send(embed=e)
        if isinstance(exception, errors.CommandOnCooldown):
            e = discord.Embed(
                description=f"{reactionFailure} You're on cooldown. Try again after {exception.retry_after:2f}.",
                color=discord.Color.red(),
            )
            return await ctx.send(embed=e)
        if isinstance(exception, errors.ChannelNotFound):
            e = discord.Embed(
                description=f"{reactionFailure} Channel was not found.",
                color=discord.Color.red(),
            )
            return await ctx.send(embed=e)
        if isinstance(exception, (errors.MemberNotFound, errors.UserNotFound)):
            e = discord.Embed(
                description=f"{reactionFailure} User/Member was not found.",
                color=discord.Color.red(),
            )
            return await ctx.send(embed=e)
        if isinstance(exception, errors.RoleNotFound):
            e = discord.Embed(
                description=f"{reactionFailure} Role was not found.",
                color=discord.Color.red(),
            )
            return await ctx.send(embed=e)
        if isinstance(exception, errors.MissingRequiredArgument) or isinstance(
            exception, errors.BadArgument
        ):
            if not ctx.command:
                return

            e = discord.Embed(
                description=f"{reactionFailure} Correct usage of the command: {ctx.command.name} {ctx.command.signature}",
                timestamp=ctx.message.created_at,
                color=discord.Color.red(),
            )
            e.set_footer(
                text="[] means the argument is optional and <> means the argument is required."
            )
            return await ctx.send(embed=e)
        ...
        if message.guild.id not in self.bot.logging_webhooks:
            async with self.bot.pool.acquire() as conn:
                res = await conn.fetchone(
                    "SELECT log_webhook, log_channel FROM guild_data WHERE guild_id = ?",
                    (message.guild.id),
                )
                ...
                self.bot.logging_webhooks[message.guild.id] = discord.Webhook.from_url(
                    res[0], session=self.bot._session
                )
    ...
    async def on_message_edit(
        ...
        if after.guild.id not in self.bot.logging_webhooks:
            async with self.bot.pool.acquire() as conn:
                ...
                self.bot.logging_webhooks[after.guild.id] = discord.Webhook.from_url(
                    res[0], session=self.bot._session
                )
        ...
        e.add_field(name="Before", value=before.content).add_field(
        ...
        try:
            await webhook.send(
                embed=e,
                username=after.guild.me.display_name,
                avatar_url=after.guild.me.display_avatar.url,
            )
        except discord.NotFound:
    ...
    async def on_member_remove(self, member: discord.Member) -> Any:
        ...
        if guild.id not in self.bot.logging_webhooks:
            async with self.bot.pool.acquire() as conn:
                ...
                self.bot.logging_webhooks[guild.id] = discord.Webhook.from_url(
                    res[0], session=self.bot._session
                )
    ...
    async def on_guild_join(self, guild: discord.Guild) -> Any:
        async with self.bot.pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO guild_data (guild_id) VALUES (?)", (guild.id)
            )
